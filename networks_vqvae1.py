...
def conv2d(x, fmaps, kernel, strides, gain=np.sqrt(2), use_wscale=False):
    ...
    w = tf.cast(w, x.dtype)
    return tf.compat.v1.nn.conv2d(x, filter=w, strides=[1, strides, strides, 1], padding='SAME', data_format='NHWC')
...
def conv2d_transpose(x, size, fmaps, kernel, strides, gain=np.sqrt(2), use_wscale=False):
    ...
    return tf.compat.v1.nn.conv2d_transpose(x, filter=w, output_shape=output_shape, strides=[1, strides, strides, 1], padding='SAME', data_format='NHWC')
...
def residual_stack(h, num_hiddens, num_residual_layers, num_residual_hiddens):
	for i in range(num_residual_layers):
		...
		with tf.compat.v1.variable_scope('res3x3_%d' % i):
			h_i = conv2d(h_i, fmaps=num_residual_hiddens, kernel=3, strides=1)
			h_i = apply_bias(h_i)
			h_i = tf.nn.relu(h_i)

		with tf.compat.v1.variable_scope('res1x1_%d' % i):
...
def encoder(x, num_hiddens, num_residual_layers, num_residual_hiddens):
  with tf.compat.v1.variable_scope('Encoder_Conv_0', reuse=tf.compat.v1.AUTO_REUSE):
    h = conv2d(x, fmaps=num_hiddens//2, kernel=4, strides=2)
    h = apply_bias(h)
    h = tf.nn.relu(h)

  with tf.compat.v1.variable_scope('Encoder_Conv_1', reuse=tf.compat.v1.AUTO_REUSE):
    h = conv2d(h, fmaps=num_hiddens, kernel=4, strides=2)
    h = apply_bias(h)
    h = tf.nn.relu(h)

  with tf.compat.v1.variable_scope('Encoder_Conv_2', reuse=tf.compat.v1.AUTO_REUSE):
    h = conv2d(h, fmaps=num_hiddens, kernel=3, strides=1)
    h = apply_bias(h)

  with tf.compat.v1.variable_scope('Encoder_residual_stack', reuse=tf.compat.v1.AUTO_REUSE):
...
def decoder(x, num_hiddens, num_residual_layers, num_residual_hiddens, image_size, num_channel):
  with tf.compat.v1.variable_scope('Decoder_Conv_0', reuse=tf.compat.v1.AUTO_REUSE):
    h = conv2d(x, fmaps=num_hiddens, kernel=3, strides=1)
    h = apply_bias(h)

  with tf.compat.v1.variable_scope('Decoder_residual_stack', reuse=tf.compat.v1.AUTO_REUSE):
    h = residual_stack(h, num_hiddens, num_residual_layers, num_residual_hiddens)
    h = tf.nn.relu(h)

  with tf.compat.v1.variable_scope('Decoder_Conv_1', reuse=tf.compat.v1.AUTO_REUSE):
    h = conv2d_transpose(h, size=image_size//2, fmaps=num_hiddens//2, kernel=4, strides=2)
    h = apply_bias(h)
    h = tf.nn.relu(h)

  with tf.compat.v1.variable_scope('Decoder_Conv_2', reuse=tf.compat.v1.AUTO_REUSE):
